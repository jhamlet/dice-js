...
var Behavior = require("dice-js/behavior"),
    Operator = require("dice-js/expression/operator"),
...
Operator.define("Low", {
    ...
    operation: function (lVal, rVal) {
        var lValue  = lVal.results,
            rValue  = rVal.valueOf(),
            len     = lValue.length,
            keep    = Math.max(Math.max(len - rValue, len), 0) + 1
        ;
        
        return lValue.sort(dutil.ascendingNumeric).slice(0, keep);
    }
});

Operator.define("High", {
    symbol: "h",
    associativity: "left",
    operation: function (lVal, rVal) {
        var lValue  = lVal.results,
            rValue  = rVal.valueOf(),
            len     = lValue.length,
            keep    = Math.max(Math.min(len - rValue, len), 0) + 1
        ;
        
        return lValue.sort(dutil.descendingNumeric).slice(0, keep);
    }
});

Operator.derive("High", "Drop", {
    symbol: "d",
    operation: function (lVal, rVal) {
        var lValue  = lVal.results,
            rValue  = rVal.valueOf(),
            len     = lValue.length,
            result  = Operator.High.prototype.operation.call(this, lVal, len - rValue)
        ;
...
Operator.derive("High", "Keep", {
    operation: function (lVal, rVal) {
        var result = Operator.High.prototype.operation.call(this, lVal, rVal);
        return result.reduce(dutil.reduceSum, 0);
    }
});
